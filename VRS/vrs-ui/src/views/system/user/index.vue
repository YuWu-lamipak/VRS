<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch">
      ...
      <el-form-item :label="$t('z')" style="margin-left: 26px" prop="phoneNumber">
        <el-input v-model="queryParams.phoneNumber" :placeholder="$t('abbb')" clearable size="small" style="width: 245px"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item :label="$t('zccc')" prop="status">
        <el-select v-model="queryParams.status" :placeholder="$t('aaacc')" clearable size="small" style="width: 245px">
      ...
      <el-form-item :label="$t('va')">
        <el-date-picker v-model="dateRange" size="small" style="width: 245px" value-format="yyyy-MM-dd" type="daterange"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['system:user:add']">{{ $t('a') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['system:user:edit']">{{ $t('f') }}</el-button>
      </el-col>
      <!--          <el-col :span="1.5">
            <el-button
              type="danger"
    ...
    <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
      ...
      <!--          <el-table-column label="黑名单" align="center" key="isBlacklist" v-if="columns[6].visible">-->
      <!--            <template slot-scope="scope">-->
      <!--              <el-switch-->
      ...
      <!--            </template>-->
      <!--          </el-table-column>-->
      <el-table-column :label="$t('va')" align="center" prop="createTime" v-if="columns[6].visible">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('lll')" align="center" width="160" class-name="small-padding fixed-width">
        <template slot-scope="scope" v-if="scope.row.userId !== 1">
          ...
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
            v-hasPermi="['system:user:remove']">{{ $t('uaa') }}</el-button>
          <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)"
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="handleResetPwd" icon="el-icon-key"
                v-hasPermi="['system:user:resetPwd']">{{ $t('uaaa') }}</el-dropdown-item>
              <el-dropdown-item command="handleAuthRole" icon="el-icon-circle-check"
                v-hasPermi="['system:user:edit']">{{ $t('zcba') }}</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="850px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="150px">
        ...
        <el-row>
          ...
          <!--          </el-col>-->
          <!--          <el-col :span="12">-->
          <!--            <el-form-item v-if="this.form.userType==='02'" label-width="90px" label="所属承运商" prop="parentId">-->
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm(this.$t('sfsc2') + '"' + name + '"?').then(function () {
