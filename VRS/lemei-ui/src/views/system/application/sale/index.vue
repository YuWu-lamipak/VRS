<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch">
      ...
      <!-- <el-form-item :label="$t('请输入交货单号')" prop="lmDnnum">
        <el-input v-model="queryParams.lmDnnum" :placeholder="$t('请输入交货单号')" clearable size="small" @keyup.enter.native="handleQuery"></el-input>
      </el-form-item>

      <el-form-item :label="$t('请输入包装ID')" prop="lmPackagingid">
        <el-input v-model="queryParams.lmPackagingid" :placeholder="$t('请输入包装ID')" clearable size="small" @keyup.enter.native="handleQuery"></el-input>
      </el-form-item> -->
    ...
    <div>
      <el-table ref="table" v-loading="loading" :data="saleList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="交货单Id" align="center" prop="lmId" v-if="false" />
        <el-table-column :label="$t('x')" align="center" prop="lmOrderid" />
        <el-table-column :label="$t('sjcc')" align="center" prop="lmActualdeliverydate" />
        <el-table-column :label="$t('ad')" align="center" prop="lmCarNumber" />
        <el-table-column :label="$t('jkk1')" align="center" prop="lmDnnum" />
        <el-table-column :label="$t('zkk1')" align="center" prop="lmDnLine" />
        <el-table-column :label="$t('bz')" align="center" prop="lmPackagingid" />
        <el-table-column :label="$t('nnz')" align="center" prop="lmGrossweight" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item :label="$t('x')" prop="lmOrderid" label-width="300px">
          <el-input style="width: 270px" v-model="form.lmOrderid" :placeholder="$t('yyhmn')" />
        </el-form-item>

        <el-form-item :label="$t('ad')" prop="lmCarNumber" label-width="300px">
          <el-input style="width: 270px" v-model="form.lmCarNumber" :placeholder="$t('ae')" readonly />
        </el-form-item>

        <el-form-item :label="$t('jkk1')" prop="lmDnnum" label-width="300px"
          :rules="[{ required: true, message: $t('lm'), trigger: ['blur', 'change'] }]">
          <el-input style="width: 270px" v-model="form.lmDnnum" :placeholder="$t('abbb')" readonly />
        </el-form-item>

        <el-form-item :label="$t('bz')" prop="lmPackagingid" label-width="300px"
          :rules="[{ required: true, message: $t('lm1'), trigger: ['blur', 'change'] }]">
          <el-input style="width: 270px" v-model="form.lmPackagingid" :placeholder="$t('addd')" readonly />
        </el-form-item>

        <el-form-item :label="$t('nn')" prop="lmGrossweight" label-width="300px"
          :rules="[{ required: true, message: $t('lm1'), trigger: ['blur', 'change'] }]">
          <el-input style="width: 270px" v-model="form.lmGrossweight" :placeholder="$t('addd')" readonly />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    //     this.ids = selection.map(item => item.lmId)
    //   }
    // },
    // 多选框选中数据
    handleSelectionChange(selection) {
    ...
    handleSaleUpdate(row) {
      this.reset();
      const lmId = row.lmId || this.ids
      getSaleApplication(lmId).then(response => {
        this.form = {
          ...response.data,
        };
        this.open = true;
    ...
    handleSaleDelete(row) {
      const lmIds = row.lmId || this.ids
      this.$modal.confirm(this.$t('joinj')).then(function () {
        return deleteSaleApplication(lmIds);
      }).then(() => {
        this.getList();
